	...
	if(g_sdio.has_thrpt_enh3) {
...
#endif
		/* select VMM table 0 */
		if((val & SEL_VMM_TBL0) == SEL_VMM_TBL0) reg |= (1 << 5);
		/* select VMM table 1 */
		if((val & SEL_VMM_TBL1) == SEL_VMM_TBL1) reg |= (1 << 6);		
		/* enable VMM */
		if((val & EN_VMM) == EN_VMM)             reg |= (1 << 7);
		if(reg) {
			sdio_cmd52_t cmd;
			cmd.read_write = 1;
			cmd.function = 0;
...
#ifdef WILC_SDIO_IRQ_GPIO
		{
			...
			if(flags) {
				...
				for(i=g_sdio.nint; i<MAX_NUM_INT; i++) {
					if(flags & 1) g_sdio.dPrint(N_ERR, "[wilc sdio]: Unexpected interrupt cleared %d...\n", i);
					flags >>= 1;	
				}
			}
		}
