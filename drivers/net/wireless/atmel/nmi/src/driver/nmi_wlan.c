...
NMI_Bool EnableTCPAckFilter = NMI_FALSE;

void Enable_TCP_ACK_Filter(NMI_Bool value)
{
	EnableTCPAckFilter = value;
}

NMI_Bool is_TCP_ACK_Filter_Enabled()
{
...
static int nmi_wlan_txq_add_net_pkt(void *priv, uint8_t *buffer, uint32_t buffer_size, nmi_tx_complete_func_t func)
{
	...
	tqe->tcp_PendingAck_index=NOT_TCP_ACK;
#ifdef TCP_ENHANCEMENTS
	if (is_TCP_ACK_Filter_Enabled() == NMI_TRUE)
#endif
	tcp_process(tqe);
#endif
	nmi_wlan_txq_add_to_tail(tqe);
		...
		**/
		...
		do {
			...
			**/
			...
			ret = p->hif_func.hif_write_reg(NMI_HOST_VMM_CTL, 0); 
			if (!ret) {
			...
			interrupt firmware
			**/
			ret = p->hif_func.hif_write_reg(0x10a8, 1);	
			if (!ret) {
				nmi_debug(N_ERR, "[nmi txq]: fail can't write reg 0x10a8..\n");
				break;
			}
			...
			**/
			...
			do {
				ret = p->hif_func.hif_read_reg(0x10a8, &reg);
				if (!ret) {
					nmi_debug(N_ERR, "[nmi txq]: fail can't read reg 0x10a8..\n");
					break;
				}
				if (reg == 0) {
					/**
...
static void nmi_pllupdate_isr_ext(uint32_t int_stats){
	...
	while(!(ISNMC3000(nmi_get_chipid(NMI_TRUE))&&--trials)) {
...
static int nmi_bt_firmware_download(const uint8_t *buffer, uint32_t buffer_size)
{
	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
	uint32_t offset;
	uint32_t addr, size, size2, blksz;
...
static int nmi_wlan_start(void)
{
...
#ifdef NMI_DISABLE_PMU
#else
	reg |= NMI_HAVE_USE_PMU;
#endif

#ifdef NMI_SLEEP_CLK_SRC_XO
	reg |= NMI_HAVE_SLEEP_CLK_SRC_XO;
#elif defined NMI_SLEEP_CLK_SRC_RTC
	reg |= NMI_HAVE_SLEEP_CLK_SRC_RTC;	
#endif

#ifdef NMI_EXT_PA_INV_TX_RX
	reg |= NMI_HAVE_EXT_PA_INV_TX_RX;
#endif
	...
	**/
	...
	if (!ret) {
		nmi_debug(N_ERR, "[nmi start]: fail read reg 0x3b0000 ...\n");
		release_bus(RELEASE_ONLY);
		/* EIO  5*/
		ret = -5;
 		return ret;
...
#ifdef DOWNLOAD_BT_FW
/* Define Modes of operation for NMC3000 */
#define WIFI_ONLY	1
#define BT_ONLY		2
#define FM_ONLY		4

static void toggle_bit_delay(uint32_t addr, uint32_t mask, uint32_t delay_ms)
{
	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
	uint32_t val32;

	p->hif_func.hif_read_reg(addr, &val32);

	if(val32 & mask)
	{
		val32 &= ~mask;
		p->hif_func.hif_write_reg(addr, val32);
		NMI_Sleep(delay_ms);
	}
	val32 |= mask;
	p->hif_func.hif_write_reg(addr, val32);
...
static void nmi_update_pll_bt(uint8_t mode, uint8_t update_cpll)
{
	uint32_t pll;

	if ((mode == 0) || (mode == WIFI_ONLY) || (mode == BT_ONLY))
	{
		//toggle_bit_delay(0x1690, 1, 50);
		toggle_bit_delay(0x3b0040, 2, 50);
		if (update_cpll == 1) {
		//	toggle_bit_delay(0x1428, 1, 50);
		}
	}
	


	if ((mode == 0) || (mode == FM_ONLY))
	{
		toggle_bit_delay(0x501070,1,50);
	}
}

static int nmi_bt_start(void)
{
	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
	uint32_t val32 = 0;
	int ret;
...
static void nmi_wlan_cleanup(void)
{
	...
	**/
	...
	ret = p->hif_func.hif_read_reg(NMI_GP_REG_0,&reg); 
	if (!ret) {
		PRINT_ER("Error while reading reg\n");
		release_bus(RELEASE_ALLOW_SLEEP);
	}
	PRINT_ER("Writing ABORT reg\n");
  	ret = p->hif_func.hif_write_reg(NMI_GP_REG_0,(reg | ABORT_INT ));  
	if (!ret) {
		PRINT_ER("Error while writing reg\n");
		release_bus(RELEASE_ALLOW_SLEEP);
	}
...
uint32_t init_chip(void)
{
	...
	printk("Bootrom sts = %x\n",reg);
		...
		**/
		...
		if (!ret) {
			nmi_debug(N_ERR, "[nmi start]: fail read reg 0x3b0090 ...\n");
			return ret;
		}
		reg |= (1 << 0);
		ret = g_wlan.hif_func.hif_write_reg(0x3b0090, reg);
		if (!ret) {
			nmi_debug(N_ERR, "[nmi start]: fail write reg 0x3b0090 ...\n");
			return ret;
		}
		...
		**/
		...
		if (!ret) {
			nmi_debug(N_ERR, "[nmi start]: fail write reg 0x4f0000 ...\n");
			return ret;
...
#ifdef DOWNLOAD_BT_FW
	oup->bt_firmware_download = nmi_bt_firmware_download;
#endif	
	oup->wlan_start = nmi_wlan_start;
#ifdef DOWNLOAD_BT_FW
	oup->bt_start = nmi_bt_start;
#endif
