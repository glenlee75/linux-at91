...
#ifdef NMC1000_BT_COEXISTENCE /* For Hexing phone */
static NMI_Uint8 u8Bluetooth_state = 0;
#endif
...
#ifdef DOWNLOAD_BT_FW
	const struct firmware* nmc_bt_firmware;
#endif
	char *firmware;
...
#ifdef DOWNLOAD_BT_FW
	if( request_firmware(&nmc_bt_firmware,BT_FIRMWARE, &g_linux_wlan->nmc_sdio_func->dev) != 0){
...
#ifdef DOWNLOAD_BT_FW
	if( request_firmware(&nmc_bt_firmware,BT_FIRMWARE, &g_linux_wlan->nmc_spidev->dev) != 0){
		PRINT_ER("%s - firmare not available. Skip\n",firmware);
	}
#endif	
#endif
...
#ifdef DOWNLOAD_BT_FW
	g_linux_wlan->nmc_bt_firmware = nmc_bt_firmware;
#endif	
...
static int linux_bt_firmware_download(linux_wlan_t* p_nic){
	...
	if(g_linux_wlan->nmc_bt_firmware == NULL){
	...
	**/
	...
	ret = g_linux_wlan->oup.bt_firmware_download(g_linux_wlan->nmc_bt_firmware->data, g_linux_wlan->nmc_bt_firmware->size);
	if(ret < 0){
		goto _FAIL_;
	}
	...
	release_firmware(g_linux_wlan->nmc_bt_firmware);
	g_linux_wlan->nmc_bt_firmware = NULL;
	...
	case SIOCSIWPRIV:
		{
			...
			if (size&& wrq->u.data.pointer) 
			{
				...
				if(strnicmp(buff,"RSSI",size) == 0) 
				{
					...
					#if 0
					//NMI_Bool bt_State = gpio_get_value(sprd_3rdparty_gpio_beken_bt_ldo_en);
					//if( bt_State != u8Bluetooth_state)
					{
						u8Bluetooth_state = bt_State;
						printk("[COEX] [DRV] setting coexistance mode to %d\n",u8Bluetooth_state);
						host_int_change_bt_coex_mode((NMI_WFIDrvHandle)gWFiDrvHandle,u8Bluetooth_state);
					}
					#endif
					//AA end
				#endif
					...
					if (copy_to_user(wrq->u.data.pointer, buff, size)) {
						...
						if(mode != 1 && mode != 0)
						{
							host_int_change_bt_coex_mode(priv->hNMIWFIDrv,mode);
						}
					#endif			        		
			        }else if(strnicmp(buff, "BTCOEXSCAN-START", strlen("BTCOEXSCAN-START")) == 0) {						
						#ifdef NMC1000_BT_COEXISTENCE
...
void NMI_WFI_mgmt_rx(uint8_t *buff, uint32_t size)
{
	...
	#ifdef NMI_P2P
	nic = netdev_priv(g_linux_wlan->strInterfaceInfo[1].nmc_netdev); //p2p0
	if( (buff[0]== nic->g_struct_frame_reg[0].frame_type && nic->g_struct_frame_reg[0].reg) ||
	   (buff[0]== nic->g_struct_frame_reg[1].frame_type && nic->g_struct_frame_reg[1].reg) )
	{
		NMI_WFI_p2p_rx(g_linux_wlan->strInterfaceInfo[1].nmc_netdev,buff,size);
	}
	#endif
}
