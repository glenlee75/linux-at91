...
extern NMI_Uint8 u8ConnectedSSID[6];
...
NMI_Uint8 u8WLANChannel = INVALID_CHANNEL ;

/*BugID_5442*/
NMI_Uint8 u8CurrChannel = 0;
							   ...
							   void* pUserVoid)
{
	...
	else if(enuConnDisconnEvent == CONN_DISCONN_EVENT_DISCONN_NOTIF)
	{
		...
		if((pstrWFIDrv->IFC_UP)&&(dev==g_linux_wlan->strInterfaceInfo[1].nmc_netdev))
		{
		...
		else if ((!pstrWFIDrv->IFC_UP)&&(dev==g_linux_wlan->strInterfaceInfo[1].nmc_netdev))
		{
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if(nic->iftype == STATION_MODE)
	{
		...
		{
			Enable_TCP_ACK_Filter(NMI_TRUE);
		}
		else if( strStatistics.u8LinkSpeed != DEFAULT_LINK_SPEED)
		{
			Enable_TCP_ACK_Filter(NMI_FALSE);
		}
#endif
...
void NMI_WFI_CfgParseRxAction(NMI_Uint8 * buf,NMI_Uint32 len)
{
	...
	NMI_Uint8 intent;
	NMI_Uint8 tie_breaker;
	NMI_Bool is_nmc_go = NMI_TRUE;
	#endif
	NMI_Uint8 op_channel_attr_index = 0;
	NMI_Uint8 channel_list_attr_index = 0;
	...
	while(index < len)
	{
		if(buf[index] == GO_INTENT_ATTR_ID)
		{
			...
			{
				PRINT_D(GENERIC_DBG, "NMC will be client (intent %d tie breaker %d)\n", intent, tie_breaker);
				is_nmc_go = NMI_FALSE;
			}
			else
			{
				PRINT_D(GENERIC_DBG, "NMC will be GO (intent %d tie breaker %d)\n", intent, tie_breaker);
				is_nmc_go = NMI_TRUE;
			}	
	...
	#ifdef USE_SUPPLICANT_GO_INTENT
	if(u8WLANChannel != INVALID_CHANNEL && is_nmc_go)
	#else
...
*  @version
*/
void NMI_WFI_CfgParseTxAction(NMI_Uint8 * buf,NMI_Uint32 len,NMI_Bool bOperChan, NMI_Uint8 iftype)
{
	NMI_Uint32 index=0;
	NMI_Uint32 i=0,j=0;
...
static void NMI_WFI_RemainOnChannelReady(void* pUserVoid)
{
	...
	priv->bInP2PlistenState = NMI_TRUE;
...
static void NMI_WFI_RemainOnChannelExpired(void* pUserVoid, NMI_Uint32 u32SessionID)
{	
	struct NMI_WFI_priv* priv;
	priv = (struct NMI_WFI_priv*)pUserVoid;	
	...
	if(u32SessionID == priv->strRemainOnChanParams.u32ListenSessionID)
	{
		...
		priv->bInP2PlistenState = NMI_FALSE;
	...
	unsigned int duration,u64 *cookie)
{
	...
	s32Error = host_int_remain_on_channel(priv->hNMIWFIDrv
										, priv->strRemainOnChanParams.u32ListenSessionID
										, duration
										, chan->hw_value
										, NMI_WFI_RemainOnChannelExpired
										, NMI_WFI_RemainOnChannelReady
										, (void *)priv);
...
*  @version	1.0
*/
static int   NMI_WFI_cancel_remain_on_channel(struct wiphy *wiphy, 
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	struct wireless_dev *wdev,
#else
	struct net_device *dev,
	...
	u64 cookie)
{
	...
	s32Error = host_int_ListenStateExpired(priv->hNMIWFIDrv, priv->strRemainOnChanParams.u32ListenSessionID);
	return s32Error;
}
/**
...
int NMI_WFI_mgmt_tx(struct wiphy *wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	struct wireless_dev *wdev,
#else
	struct net_device *dev,
									...
									for(i=P2P_PUB_ACTION_SUBTYPE+2;i<len;i++)
									{
								       	if(buf[i]== P2PELEM_ATTR_ID && !(NMI_memcmp(u8P2P_oui,&buf[i+2],4)) )
										{
											if(buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_REQ || buf[P2P_PUB_ACTION_SUBTYPE]==P2P_INV_RSP)
												NMI_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),NMI_TRUE, nic->iftype); 
											...
											/*to parse transmitted negotiation frames*/
												...
												NMI_WFI_CfgParseTxAction(&mgmt_tx->buff[i+6],len-(i+6),NMI_FALSE, nic->iftype); 
											#endif
											break;
										}
									}
...
{	
	...
	if(priv->bInP2PlistenState == NMI_FALSE)
	{
...
*  @version
*/
void    NMI_WFI_frame_register(struct wiphy *wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
	struct wireless_dev *wdev,
#else
	struct net_device *dev,
	...
	enum nl80211_iftype type, u32 *flags,struct vif_params *params)
{
	...
	case NL80211_IFTYPE_STATION:
		...
		if(g_linux_wlan->nmc1000_initialized)
		{
			host_int_del_All_Rx_BASession(priv->hNMIWFIDrv, g_linux_wlan->strInterfaceInfo[0].aBSSID, TID);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0)
	s32Error = NMI_WFI_CfgSetChannel(wiphy, &settings->chandef);
