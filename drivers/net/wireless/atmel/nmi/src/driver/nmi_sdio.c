...
static int sdio_deinit(void *pv)
{
	...
	**/
	...
	cmd.data = 0x8;
	if (!g_sdio.sdio_cmd52(&cmd)) {
		g_sdio.dPrint(N_ERR, "[nmi sdio]: Fail cmd 52, reset cmd ...\n");
			}
#endif
...
static int sdio_init(nmi_wlan_inp_t *inp, nmi_debug_func func)
{
	...
	**/
	...
	g_sdio.dPrint(N_ERR, "[nmi sdio]: has_thrpt_enh3 = %d...\n", g_sdio.has_thrpt_enh3);
...
static int sdio_read_int(uint32_t * int_status)
{
	...
	**/	
	...
	cmd.function = 1;
	//cmd.raw = 0;
	cmd.address = 0x04;	//AA: register 4?
	cmd.data = 0;
	g_sdio.sdio_cmd52(&cmd);
	//printk("%s @ %d: sdio read %x = %x\n", __FUNCTION__, __LINE__, cmd.address, cmd.data);
	if(cmd.data & (1 << 0)) {
...
static int sdio_clear_int_ext(uint32_t val)
{
	...
	if(g_sdio.has_thrpt_enh3) {
		...
		reg = 0;
		/* select VMM table 0 */
		if((val & SEL_VMM_TBL0) == SEL_VMM_TBL0) reg |= (1 << 0);
		/* select VMM table 1 */
		if((val & SEL_VMM_TBL1) == SEL_VMM_TBL1) reg |= (1 << 1);		
		/* enable VMM */
		if((val & EN_VMM) == EN_VMM)             reg |= (1 << 2);
		if(reg) {
			sdio_cmd52_t cmd;
			cmd.read_write = 1;
			cmd.function = 0;
